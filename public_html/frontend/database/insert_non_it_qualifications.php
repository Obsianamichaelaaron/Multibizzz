<?php
/**
 * Insert 10 Non-IT Qualifications
 * Run this script to populate the qualifications table with 10 non-IT qualifications
 * This ensures the chatbot supports all job categories, not just IT
 */
...
// Non-IT Qualifications - Marketing, Accounting, Business, Human Resources
...
foreach ($non_it_qualifications as $qual) {
    // Check if qualification already exists
    $check_stmt = $conn->prepare("SELECT qualification_id, description FROM qualifications WHERE name = ?");
    $check_stmt->bind_param("s", $qual['name']);
    $check_stmt->execute();
    $result = $check_stmt->get_result();
    
    if ($result->num_rows > 0) {
        // Update description if it exists but description might be different
        $existing = $result->fetch_assoc();
        if ($existing['description'] !== $qual['description']) {
...
echo "========================================\n";
echo "\n✓ Non-IT qualifications setup completed!\n";
echo "The chatbot now supports all job categories.\n";
